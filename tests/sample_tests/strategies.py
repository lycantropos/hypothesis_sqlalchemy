...
dialects = strategies.one_of(
    [
        strategies.builds(dialect)
        for dialect in [
            DefaultDialect,
            mysql.dialect,
            postgresql.dialect,
...
max_table_records_lists_sizes = strategies.none() | strategies.integers(
    MAX_MIN_SIZE + 1, MAX_SIZE
)
...
def fix_columns_values(
    ...
    def to_item(column: Column[Any]) -> tuple[str, Strategy[Any]]:
