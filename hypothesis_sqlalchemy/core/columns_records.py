...
def lists(
    ...
    **fixed_columns_values: Strategy[Any],
    ...
    unique_indices = [
        [columns_indices[column] for column in constraint.columns]
        for constraint in constraints
        if isinstance(constraint, table_constraints.UNIQUE_TYPES)
        and constraint.columns
    ]
    unique_by = (
        tuple(starmap(itemgetter, unique_indices)) if unique_indices else None
    )
    return strategies.lists(
        instances(columns, **fixed_columns_values),
        min_size=min_size,
        max_size=max_size,
        unique_by=unique_by,
    )
